...
export default function AllMovies() {
  ...
  // Fetch movies when page changes or search is performed
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      try {
        ...
        // For TMDB API, each page typically has 20 results, with 500 pages maximum
        setTotalPages(20); // Default to 20 pages if not available from API
      } catch (err) {
  ...
  // Safety check for rendering movies
  const renderMovies = Array.isArray(movies) ? movies : [];
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className={`text-3xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              {isSearchActive ? `Resultados para "${searchQuery}"` : 'Todas las Películas'}
            </h1>
            <Link 
              href="/dashboard"  
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {renderMovies.map((movie) => (
                ...
                >
                  <Link href={`/movie/${movie.id}`} className="block h-64 relative">
